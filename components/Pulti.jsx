...
export default function Pulti() {
  ...
  return (
    <div className="min-h-screen bg-gray-100 pb-24">
      ...
      {pogled === "prevzem" && (
        <PrevzemniZapisnikPulti
          nalog={nalogi.find((x) => x.id === izbran)}
          nazaj={() => setPogled("podrobnosti")}
          shraniPrevzem={(nal, podatkiPodpisa, ime, pripombe) => {
            shraniNaloge(
              nalogi.map((x) =>
                x.id === nal.id
                  ? {
                      ...x,
                      prevzemPodpis: podatkiPodpisa,
                      prevzemIme: ime,
                      prevzemDatum: new Date().toISOString(),
                      prevzemPripombe: pripombe || "",
                    }
                  : x
              )
            );
...
const NAVODILA_VZDRZEVANJE = [
  "Ne režite z ostrimi predmeti (noži, škarje) neposredno na pultu — vedno uporabljajte desko za rezanje.",
  "Vročih posod, ponev ali pekačev ne odlagajte neposredno na površino — vedno uporabljajte podstavke.",
  "Mastnih in obarvanih tekočin (olje, vino, kava, sokovi, kis, limona) ne puščajte na površini — razlitje takoj obrišite.",
  "Ne uporabljajte agresivnih čistil (kisline, razredčila, abrazivna sredstva, groba gobica) — čistite z blago milnico in mehko krpo.",
  "Na pult ne sedajte, ne stopajte in nanj ne odlagajte zelo težkih predmetov — kamen ni odporen na točkovne obremenitve.",
  "Udarci s trdimi ali kovinskimi predmeti lahko poškodujejo robove in vogale.",
  "Za naravni kamen priporočamo občasno impregnacijo (zaščito) površine — za termin se lahko obrnete na nas.",
];
  ...
  return (
    <div className="p-3 space-y-3">
      <style>{`
        @media print {
          body * { visibility: hidden; }
          .prevzem-pulti, .prevzem-pulti * { visibility: visible; }
          .prevzem-pulti { position: absolute; top: 0; left: 0; width: 100%; padding: 0; margin: 0; }
          .prevzem-pulti-brez { display: none !important; }
        }
      `}</style>
      ...
      <div className="prevzem-pulti bg-white rounded-xl p-4 sm:p-6 border border-gray-200 text-sm">
        <div className="flex items-center justify-between border-b-2 border-black pb-2 mb-3">
          <div className="font-bold text-lg">ČAKŠ <span className="text-red-600">· Pulti</span></div>
          <div className="flex items-center gap-3">
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-1 border-b border-gray-200 pb-2 mb-3">
          ...
          <div><span className="text-xs text-gray-400 uppercase mr-1">Stranka:</span><span className="font-semibold">{nalog.stranka?.ime}</span></div>
          {nalog.stranka?.naslov && <div><span className="text-xs text-gray-400 uppercase mr-1">Lokacija:</span>{nalog.stranka.naslov}</div>}
        </div>

        <div className="mb-4">
          <div className="text-xs text-gray-400 uppercase mb-1.5 font-semibold">Izjava stranke ob prevzemu</div>
          <p className="leading-relaxed">
            Stranka izjavlja, da je ob prevzemu <strong>pregledala kuhinjske pulte</strong> in ugotovila:
          </p>
          <ul className="list-disc pl-5 mt-1.5 space-y-1 leading-relaxed">
            <li>da <strong>vse mere ustrezajo</strong> naročenim in dogovorjenim,</li>
            <li>da <strong>vse odprtine in izrezi</strong> (korito, steklokeramika, vtičnice in drugo) ustrezajo ter so pravilno izvedeni,</li>
            <li>da na izdelkih <strong>ni vidnih poškodb</strong>, razpok ali napak,</li>
            <li>da je izvedeno delo <strong>v skladu z dogovorom</strong>.</li>
          </ul>
          <p className="mt-2 leading-relaxed">
            Stranka potrjuje, da je bila <strong>seznanjena z navodili za uporabo in vzdrževanje</strong> (navedena spodaj)
            ter da s svojim podpisom <strong>potrjuje prevzem izdelkov brez pripomb</strong>.
          </p>
        </div>

        <div className="mb-4 bg-gray-50 rounded-lg p-3">
          <div className="text-xs text-gray-400 uppercase mb-1.5 font-semibold">Navodila za uporabo in vzdrževanje</div>
          <ul className="list-disc pl-5 space-y-1 leading-relaxed text-[13px]">
            {NAVODILA_VZDRZEVANJE.map((navodilo, i) => (
              <li key={i}>{navodilo}</li>
            ))}
          </ul>
          <p className="text-[11px] text-gray-500 mt-2">
            Poškodbe, nastale zaradi neupoštevanja zgornjih navodil, niso predmet reklamacije.
          </p>
        </div>
      ...
      {podpisovanje && (
        <PodpisniPadPulti
          ...
          onPreklici={() => setPodpisovanje(false)}
          onShrani={(podatkiPodpisa, ime) => {
            shraniPodpisPrevzema(nalog, podatkiPodpisa, ime);
            setPodpisovanje(false);
          }}
        />
      )}
...
const NAVODILA_VZDRZEVANJE = [
  "Ne režite neposredno na pultu z noži ali drugimi ostrimi predmeti — vedno uporabljajte desko za rezanje.",
  "Vročih loncev in ponev ne odlagajte neposredno na pult — vedno uporabljajte podstavke.",
  "Razlitja (olje, maščoba, vino, kava, citrusi, kis) takoj obrišite — kisline in maščobe lahko pustijo trajne madeže.",
  "Za čiščenje uporabljajte blaga čistila in mehko krpo. Ne uporabljajte agresivnih kemikalij, razredčil, belil ali grobih žičnih gobic.",
  "Na pult ne sedajte, ne stopajte in nanj ne odlagajte zelo težkih bremen — nevarnost loma, zlasti ob izrezih in previsih.",
  "Izogibajte se močnim udarcem s trdimi predmeti, še posebej po robovih in vogalih.",
  "Naravni kamen priporočamo občasno zaščititi z impregnacijo (po potrebi, glede na uporabo).",
];

function PrevzemniZapisnikPulti({ nalog, nazaj, shraniPrevzem }) {
  const [podpisovanje, setPodpisovanje] = useState(false);
  const [pripombe, setPripombe] = useState(nalog?.prevzemPripombe || "");
  if (!nalog) return <div className="p-4">Nalog ne obstaja. <button onClick={nazaj} className="text-red-600 underline">Nazaj</button></div>;
  ...
  return (
    <div className="p-3 space-y-3">
      ...
      <div className="prevzem-pulti-brez flex flex-wrap gap-2">
        <button
          onClick={() => prenesiHTMLDokumentPulti(".prevzem-pulti", `Prevzemni zapisnik ${nalog.stevilka || ""}`, `prevzemni-zapisnik-${nalog.stevilka || "pult"}${strankaVarno ? " " + strankaVarno : ""}.html`)}
          className="bg-gray-800 text-white px-4 py-2.5 rounded-xl text-sm font-semibold"
        >
          Prenesi / natisni datoteko
        </button>
      ...
      {!nalog.prevzemPodpis && (
        <div className="prevzem-pulti-brez bg-white rounded-xl p-3">
          <label className="text-xs text-gray-500 mb-1 block">Pripombe ob prevzemu (neobvezno — prikažejo se na zapisniku)</label>
          <textarea
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white"
            rows={2}
            value={pripombe}
            onChange={(e) => setPripombe(e.target.value)}
            placeholder="npr. manjša praska na levem robu — dogovorjeno popravilo"
          />
        </div>
      )}
      ...
      <div className="prevzem-pulti bg-white rounded-xl p-4 sm:p-6 border border-gray-200 text-sm">
        <div className="flex items-center justify-between border-b-2 border-black pb-2 mb-3">
          <div className="font-bold text-lg">ČAKŠ <span className="text-red-600">· Pulti</span></div>
          <div className="flex items-center gap-3">
            <div className="text-sm uppercase font-semibold text-gray-600">Prevzemni zapisnik</div>
            <img
              src={`https://api.qrserver.com/v1/create-qr-code/?size=70x70&data=${encodeURIComponent(`https://delavni-nalog-caks.vercel.app/status/${nalog.id}`)}`}
              alt="QR koda za status"
              width={70}
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-1 border-b border-gray-200 pb-2 mb-3">
          <div><span className="text-xs text-gray-400 uppercase mr-1">Št. naloga:</span><span className="font-semibold">{nalog.stevilka}</span></div>
          <div><span className="text-xs text-gray-400 uppercase mr-1">Datum prevzema:</span>{nalog.prevzemDatum ? new Date(nalog.prevzemDatum).toLocaleDateString("sl-SI") : new Date().toLocaleDateString("sl-SI")}</div>
          <div><span className="text-xs text-gray-400 uppercase mr-1">Stranka:</span><span className="font-semibold">{nalog.stranka?.ime}</span></div>
          {nalog.stranka?.telefon && <div><span className="text-xs text-gray-400 uppercase mr-1">Tel:</span>{nalog.stranka.telefon}</div>}
          {nalog.stranka?.naslov && <div className="col-span-2"><span className="text-xs text-gray-400 uppercase mr-1">Naslov montaže:</span>{nalog.stranka.naslov}</div>}
        </div>

        <div className="mb-3">
          <div className="text-xs text-gray-400 uppercase mb-1">Izjava stranke ob prevzemu</div>
          <p className="mb-2">
            Podpisana stranka izjavljam, da sem ob prevzemu pregledal/a vgrajene kuhinjske pulte in potrjujem naslednje:
          </p>
          <ul className="space-y-1 pl-1">
            <li>✓ Vse mere pultov ustrezajo naročilu in dogovoru.</li>
            <li>✓ Vse odprtine in izrezi (korito, kuhalna plošča, vtičnice, armatura) ustrezajo in so pravilno izvedeni.</li>
            <li>✓ Na izdelkih ob prevzemu ni vidnih poškodb, razpok ali napak.</li>
            <li>✓ Izvedba, material in obdelava so v skladu z dogovorom in ponudbo.</li>
            <li>✓ Seznanjen/a sem z navodili za uporabo in vzdrževanje, navedenimi spodaj.</li>
          </ul>
          <p className="mt-2">
            S podpisom potrjujem, da je prevzem opravljen in da je vse v skladu z dogovorom
            {(nalog.prevzemPripombe || pripombe) ? ", z izjemo pripomb, navedenih spodaj." : ", brez pripomb."}
          </p>
        </div>

        {(nalog.prevzemPripombe || pripombe) && (
          <div className="mb-3 bg-yellow-50 border border-yellow-200 rounded-lg p-2">
            <span className="text-xs text-gray-400 uppercase block mb-0.5">Pripombe ob prevzemu</span>
            {nalog.prevzemPripombe || pripombe}
          </div>
        )}

        <div className="mb-3">
          <div className="text-xs text-gray-400 uppercase mb-1">Navodila za uporabo in vzdrževanje kamnitih pultov</div>
          ...
          <p className="text-xs text-gray-500 mt-2">
            Poškodbe, nastale zaradi neupoštevanja zgornjih navodil, niso predmet garancije oziroma reklamacije.
          </p>
        </div>
        ...
        {nalog.prevzemPodpis ? (
          <div className="grid grid-cols-2 gap-4 sm:gap-8 mt-6 pt-2">
            <div>
              <p className="text-xs text-gray-400 uppercase mb-1">Predal (Kamnoseštvo Čakš)</p>
              <div className="border-b border-gray-400 h-12" />
            </div>
            <div>
              <p className="text-xs text-gray-400 uppercase mb-1">Prevzel/a in potrdil/a: {nalog.prevzemIme}</p>
              <img src={nalog.prevzemPodpis} alt="Podpis stranke" className="h-14 object-contain" />
              <p className="text-[10px] text-gray-400">Podpisano: {nalog.prevzemDatum ? new Date(nalog.prevzemDatum).toLocaleString("sl-SI") : ""}</p>
            </div>
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-4 sm:gap-8 mt-6 pt-2">
            ...
            <div>
              <p className="text-xs text-gray-400 uppercase mb-1">Prevzel/a in potrdil/a (podpis stranke)</p>
              <div className="border-b border-gray-400 h-12" />
            </div>
          </div>
        )}
      ...
      {podpisovanje && (
        <PodpisniPadPulti
          ...
          onPreklici={() => setPodpisovanje(false)}
          onShrani={(podatkiPodpisa, ime) => {
            shraniPrevzem(nalog, podatkiPodpisa, ime, pripombe);
            setPodpisovanje(false);
          }}
        />
      )}
