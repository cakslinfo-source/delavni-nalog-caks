...
function izvoziDonatoniCSV(nalog) {
  ...
  const zadnjiDelStevilke = (nalog.stevilka || "").split("-").pop() || "";
  const oznakaNaloga = `${prveTriCrkeStranke}${zadnjiDelStevilke}`;
  const vrstice = [];
  postavke.forEach((p, idx) => {
    segmentiPostavke(p, idx).forEach((seg) => {
      const imeOsnova = seg.oznaka
        ? `${oznakaNaloga} ${seg.imePolice} ${seg.oznaka} (celo ${seg.celaDolzinaMM}x${seg.sirinaMM})`
        : `${oznakaNaloga} ${seg.imePolice} ${seg.dolzinaMM}x${seg.sirinaMM}`;
...
export default function DelovniNalogi() {
  ...
  async function posodobiObvestilo(transformFn) {
    ...
    try {
      ...
      if (!shraniRes.ok) {
        alert("Shranjevanje ni uspelo. Preveri povezavo.");
        return false;
      }
      setObvestilo(novo);
      if (odgovor.verzija !== undefined) setObvestiloVerzija(odgovor.verzija);
      return true;
    } catch (e) {
      alert("Napaka pri shranjevanju. Preveri povezavo.");
      return false;
    } finally {
      setShranjujeObvestilo(false);
    }
  }
  ...
  const podatkiPrevzel = stevecPoPolju("izvajalec");
        ...
        <div className="max-w-4xl mx-auto px-5 py-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <a
              ...
              className="text-stone-400 hover:text-white text-xs border border-stone-700 rounded px-2.5 py-1.5 hover:bg-stone-800 transition-colors"
              title="Pregled proizvodnje za pisarno (odpre v novem zavihku)"
            >
              📺 Pregled
            </a>
            <button
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="text-sm px-3 py-1.5 rounded-lg bg-amber-600 text-white font-medium disabled:opacity-60"
                      >
                        {shranjujeObvestilo ? "Shranjujem …" : "Objavi obvestilo"}
                      </button>
                      <button
                        onClick={() => setUrejanjeObvestila(false)}
                        className="text-sm px-3 py-1.5 rounded-lg text-stone-500"
                      >
                        Prekliči
                      </button>
                    </div>
                  ...
                  <>
                    ...
                    {obvestilo?.trenutno?.besedilo && (
                      <>
                        ...
                        <div className="flex flex-wrap gap-2 items-center border-t border-amber-200 pt-2">
                          <select
                          ...
                          <input
                            ...
                            className="flex-1 min-w-[140px] text-sm px-3 py-1.5 rounded-lg border border-amber-300"
                          />
                          <button
                            onClick={async () => {
                              if (!avtorKomentarja) {
